...
test('an editor can record a standalone conjecture for a segment', function () {
    $this->actingAs(User::factory()->editor()->create());
    $segment = Segment::factory()->create();

    $response = $this->post(route('conjectures.store', $segment), [
        'text' => 'οἰωνοῖσΐ',
        'note' => 'metri causa',
    ]);
    ...
    expect($conjecture->segment_id)->toBe($segment->id)
...
test('a conjecture can record who first proposed it and where it was published, distinct from who entered it', function () {
    ...
    $this->post(route('conjectures.store', $segment), [
        'text' => 'οἰωνοῖσΐ',
        'proposed_by' => 'Bentley',
    ]);
...
test('proposed_by is optional', function () {
    ...
    $response = $this->post(route('conjectures.store', $segment), ['text' => 'reading']);
...
test('a conjecture defaults to a plain substitution', function () {
    ...
    $this->post(route('conjectures.store', $segment), ['text' => 'reading']);
...
test('several supplements, from different proposers, can target the same lacuna', function () {
    $this->actingAs(User::factory()->editor()->create());
    $segment = Segment::factory()->create();
    $lacuna = Conjecture::factory()->for($segment, 'segment')->lacuna()->create();
...
test('a supplement cannot target a lacuna belonging to a different segment', function () {
    $this->actingAs(User::factory()->editor()->create());
    $segment = Segment::factory()->create();
...
test('a transposition can be catalogued for a work without applying it to any edition', function () {
    ...
    [$start, $end, $target] = Segment::factory()->for($work)->count(3)->sequence(
    ...
    $this->post(route('conjectures.store', $start), [
        'type' => 'transposition',
        'transposition_range_end_segment_id' => $end->id,
        'move_target_segment_id' => $target->id,
        'move_position' => 'after',
        'proposed_by' => 'Bentley',
    ])->assertRedirect();
    ...
    expect($conjecture->type)->toBe(ConjectureType::Transposition)
        ->and($conjecture->move_target_segment_id)->toBe($target->id)
    ...
    $this->post(route('conjectures.store', $start), [
        'type' => 'transposition',
        'transposition_range_end_segment_id' => $target->id,
        'move_target_segment_id' => $end->id,
        'move_position' => 'after',
    ])->assertInvalid(['move_target_segment_id']);
});

test('a reordering can be catalogued for a contiguous stretch, hanging from its first segment', function () {
    $this->actingAs(User::factory()->editor()->create());
    $work = Work::factory()->create();
    [$one, $two, $three] = Segment::factory()->for($work)->count(3)->sequence(
        ['sort_key' => '00000001', 'label' => '1'],
        ['sort_key' => '00000002', 'label' => '2'],
        ['sort_key' => '00000003', 'label' => '3'],
    )->create();
    ...
    $this->post(route('conjectures.store', $two), [
        'type' => 'reordering',
        'segment_ids' => [$three->id, $one->id, $two->id],
        'proposed_by' => 'Bergk',
    ])->assertRedirect();

    $conjecture = Conjecture::sole();
    expect($conjecture->type)->toBe(ConjectureType::Reordering)
        ->and($conjecture->segment_id)->toBe($one->id)
        ->and($conjecture->orderingEntries()->orderBy('sequence')->pluck('segment_id')->all())->toBe([$three->id, $one->id, $two->id]);

    $this->post(route('conjectures.store', $one), [
        'type' => 'reordering',
        'segment_ids' => [$three->id, $one->id],
    ])->assertInvalid(['segment_ids']);
});
...
test('editing keeps the record consistent with its kind, and a placed conjecture cannot change kind', function () {
    ...
    $conjecture = Conjecture::factory()->for($one, 'segment')->create(['type' => ConjectureType::Substitution, 'text' => 'verbum']);
    ...
    $this->patch(route('conjectures.update', $conjecture), [
        'type' => 'reordering',
        'segment_ids' => [$two->id, $one->id, $three->id],
    ])->assertRedirect();
    ...
    $placed = Conjecture::factory()->for($one, 'segment')->create(['type' => ConjectureType::Substitution, 'text' => 'aliud']);
    LemmaReading::factory()->create(['conjecture_id' => $placed->id, 'transcription_layer_id' => null, 'start_offset' => null, 'end_offset' => null, 'lemma_id' => Lemma::factory()->for($one, 'segment')->create()->id]);
...
test('the work page lists every conjecture with where it is in use', function () {
    $this->actingAs(User::factory()->editor()->create());
    $work = Work::factory()->create();
    $segment = Segment::factory()->for($work)->create(['label' => '7']);
    $conjecture = Conjecture::factory()->for($segment, 'segment')->create(['type' => ConjectureType::Substitution, 'text' => 'verbum', 'proposed_by' => 'Dover']);

    $this->get(route('works.show', $work))
        ->assertInertia(fn (AssertableInertia $page) => $page
            ->has('conjectures', 1)
            ->where('conjectures.0.id', $conjecture->id)
            ->where('conjectures.0.segment_label', '7')
