 ...
 * EditionOrderTest's addSegmentsToEdition().
 */
...
test('authoring a new reordering conjecture creates it, its entries, and selects it for the edition in one step', function () {
    ...
    ['work' => $work, 'edition' => $edition, 'segments' => $segments] = editionForOrdering(3);

    $response = $this->post(route('conjecture-orderings.store', $edition), [
        'segment_ids' => [$segments[2]->id, $segments[0]->id, $segments[1]->id],
        'proposed_by' => 'Bergk',
    ]);
    ...
    expect($conjecture->type)->toBe(ConjectureType::Reordering)
        ...
        ->and($conjecture->segment_id)->toBe($segments[0]->id) // first by numbering order
        ->and($conjecture->orderingEntries->pluck('segment_id')->all())->toBe([$segments[2]->id, $segments[0]->id, $segments[1]->id]);
    ...
    $show->assertInertia(fn (AssertInertia $page) => $page
        ->where('windowSegments.0.label', $segments[2]->label)
        ->where('windowSegments.1.label', $segments[0]->label)
        ->where('windowSegments.2.label', $segments[1]->label));
});
...
test('a proposal recorded without following is catalogued as a candidate and leaves the order alone', function () {
    ...
    $conjecture = Conjecture::sole();
    expect($conjecture->orderingEntries->pluck('segment_id')->all())
        ->toBe([$segments[2]->id, $segments[0]->id, $segments[1]->id])
    ...
    $this->get(route('editions.show', [$work, $edition]))
        ->assertInertia(fn (AssertInertia $page) => $page
            ->where('windowSegments.0.label', $segments[0]->label)
            ->where('windowSegments.1.label', $segments[1]->label)
            ->where('windowSegments.2.label', $segments[2]->label)
            ->where('windowSegments.0.order_range.candidates', fn ($candidates) => collect($candidates)
                ->contains(fn (array $candidate) => $candidate['conjecture_id'] === $conjecture->id
                    && $candidate['matches_current'] === false)));
});

test('the submitted segments must form one contiguous range, nothing left out', function () {
    $this->actingAs(User::factory()->editor()->create());
    ['edition' => $edition, 'segments' => $segments] = editionForOrdering(3);

    // Leaves out the middle segment — not a contiguous range.
    $response = $this->post(route('conjecture-orderings.store', $edition), [
        'segment_ids' => [$segments[2]->id, $segments[0]->id],
    ]);

    $response->assertInvalid(['segment_ids']);
    expect(Conjecture::count())->toBe(0);
});

test('at least two segments are required', function () {
    $this->actingAs(User::factory()->editor()->create());
    ['edition' => $edition, 'segments' => $segments] = editionForOrdering(2);
...
test('an arrangement that divides a line is registered as pieces and reported like a witness\'s split assignment', function () {
    $this->actingAs(User::factory()->editor()->create());
    ['work' => $work, 'edition' => $edition, 'segments' => $segments] = editionForOrdering(2);

    // The editor cut the tail of line 1 and pasted it after line 2.
    $this->post(route('conjecture-orderings.store', $edition), [
        'pieces' => [
            ['segment_id' => $segments[0]->id, 'part' => 1, 'text' => 'the quick'],
            ['segment_id' => $segments[1]->id, 'part' => 1, 'text' => 'line two'],
            ['segment_id' => $segments[0]->id, 'part' => 2, 'text' => 'fox'],
        ],
        'proposed_by' => 'Bentley',
        'follow' => false,
    ])->assertRedirect();
    ...
    $conjecture = Conjecture::sole();
    expect($conjecture->orderingEntries->map(fn ($entry) => [$entry->segment_id, $entry->part, $entry->text])->all())
        ->toBe([[$segments[0]->id, 1, 'the quick'], [$segments[1]->id, 1, 'line two'], [$segments[0]->id, 2, 'fox']]);
    ...
    expect($report)->toHaveCount(1)
        ...
        ->and($report[0]['parts'])->toBe([['part' => 1, 'after_label' => null], ['part' => 2, 'after_label' => $segments[1]->label]])
        ->and($report[0]['statements'])->toBe([sprintf('Bentley (conjecture): %s 2/2 "fox" stands after %s', $segments[0]->label, $segments[1]->label)])
...
test('every part of a divided line must be present, with its words', function () {
    ...
    // Part 2 without part 1: the segment is incomplete.
 ...
 *
 * @return array{work: Work, edition: Edition, segments: array{0: Segment, 1: Segment}}
 */
function editionWithWordyLines(): array
{
    ...
    foreach ([[1, 0, 13], [2, 14, 22]] as [$line, $start, $end]) {
        ...
        $assignment = Assignment::factory()->for($layer)->for($segment, 'segment')->create(['start_offset' => $start, 'end_offset' => $end]);
        SegmentAdder::add($edition, $assignment, (float) $line);
        $segments[] = $segment;
    }
...
test('adopting an arrangement that divides a line prints the line in pieces, in the proposed order', function () {
    $this->actingAs(User::factory()->editor()->create());
    ['work' => $work, 'edition' => $edition, 'segments' => $segments] = editionWithWordyLines();
    ...
    $rows = EditionSegment::where('edition_id', $edition->id)->orderBy('position')->get();
    expect($rows->map(fn (EditionSegment $row) => [$row->segment_id, $row->part, $row->part_text])->all())
        // A line read whole keeps no words of its own; only the parts do.
        ->toBe([[$segments[0]->id, 1, 'the quick'], [$segments[1]->id, 1, null], [$segments[0]->id, 2, 'fox']])
    ...
    expect($printed)->toBe([
        [$segments[0]->label, 1, 2, 0, 1, false, 'the quick'],
        [$segments[1]->label, 1, 1, 0, 1, false, 'line two'],
        [$segments[0]->label, 2, 2, 2, 2, false, 'fox'],
    ]);
...
test('adopting a registered arrangement from the line notice divides the line, and adopting a whole-line one rejoins it', function () {
    $this->actingAs(User::factory()->editor()->create());
    ['edition' => $edition, 'segments' => $segments] = editionWithWordyLines();
    ...
    expect(EditionSegment::where('edition_id', $edition->id)->count())->toBe(2);

    $this->post(route('edition-adoptions.store', $edition), ['conjecture_id' => $divided->id])->assertRedirect();
    expect(EditionSegment::where('edition_id', $edition->id)->count())->toBe(3)
    ...
    $rows = EditionSegment::where('edition_id', $edition->id)->orderBy('position')->get();
    expect($rows->map(fn (EditionSegment $row) => [$row->segment_id, $row->part, $row->part_text])->all())
        ->toBe([[$segments[1]->id, 1, null], [$segments[0]->id, 1, null]]);
});
...
test('whole segments as pieces are adopted like any reordering', function () {
    $this->actingAs(User::factory()->editor()->create());
    ['edition' => $edition, 'segments' => $segments] = editionForOrdering(2);

    $this->post(route('conjecture-orderings.store', $edition), [
        'pieces' => [
            ['segment_id' => $segments[1]->id, 'part' => 1, 'text' => 'word'],
            ['segment_id' => $segments[0]->id, 'part' => 1, 'text' => 'word'],
        ],
    ])->assertRedirect();
