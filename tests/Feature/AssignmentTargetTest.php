...
test('marking a span creates it already assigned, in one step', function () {
    ...
    expect($assignment->segment->work_id)->toBe($work->id)
        ->and($assignment->segment->label)->toBe('1.1')
...
test('an assignment can be assigned with an alphanumeric line label like "4a"', function () {
    ...
    $assignment = $transcription->assignments()->sole();
    expect($assignment->segment->label)->toBe('1.4a')
        ->and($assignment->segment->address)->toBe(['book' => 1, 'line' => '4a']);
});
...
test('an alphanumeric line label sorts between its numeric neighbours', function () {
    ...
    $scheme = ReferenceScheme::factory()->create();
    $work = Work::factory()->for($scheme, 'referenceScheme')->create();
    $four = Segment::factory()->for($work)->create(['address' => ['book' => 1, 'line' => 4], 'sort_key' => '00000001.00000004', 'label' => '1.4']);
    $five = Segment::factory()->for($work)->create(['address' => ['book' => 1, 'line' => 5], 'sort_key' => '00000001.00000005', 'label' => '1.5']);
    ...
    $fourA = $assignment->fresh()->segment;
    expect(strcmp($four->sort_key, $fourA->sort_key))->toBeLessThan(0)
        ->and(strcmp($fourA->sort_key, $five->sort_key))->toBeLessThan(0);
});

test('an assignment can be assign afreshd to a different work, creating the segment', function () {
    $this->actingAs(User::factory()->editor()->create());
    $scheme = ReferenceScheme::factory()->create();
    $work = Work::factory()->for($scheme, 'referenceScheme')->create();
    $assignment = Assignment::factory()->create();
    ...
    $assignment->refresh();
    expect($assignment->segment)->not->toBeNull()
        ->and($assignment->segment->work_id)->toBe($work->id)
        ->and($assignment->segment->label)->toBe('1.1')
        ->and($work->relatedWitnesses()->whereKey($assignment->transcriptionLayer->transcription->witness_id)->exists())->toBeTrue();
});

test('re-assigning text to an assignment reuses an existing segment for the same assignment', function () {
    $this->actingAs(User::factory()->editor()->create());
    $scheme = ReferenceScheme::factory()->create();
    $work = Work::factory()->for($scheme, 'referenceScheme')->create();
    ...
    expect($assignment->fresh()->segment_id)->toBe($segment->id)
        ->and($work->segments()->count())->toBe(1);
});

test('an assignment\'s assignment cannot be cleared — a work_id is always required', function () {
    $this->actingAs(User::factory()->editor()->create());
    $segment = Segment::factory()->create();
    $assignment = Assignment::factory()->for($segment, 'segment')->create();
...
test('assigning an assignment that does not match the work\'s numbering scheme fails', function () {
    ...
    $work = Work::factory()->for($scheme, 'referenceScheme')->create();
    $assignment = Assignment::factory()->create();
    $originalSegmentId = $assignment->segment_id;
    ...
    expect($assignment->fresh()->segment_id)->toBe($originalSegmentId);
});
...
test('a work another transcript of the same witness already holds cannot be assigned here', function () {
    ...
    Assignment::factory()->for($firstLayer)->for($segment, 'segment')->create(['start_offset' => 0, 'end_offset' => 3]);
    ...
    $otherSegment = Segment::factory()->for($other)->create(['address' => ['book' => 1, 'line' => 1], 'sort_key' => '00000001.00000001', 'label' => '1.1']);
    $assignment = Assignment::factory()->for($secondLayer)->for($otherSegment, 'segment')->create(['start_offset' => 0, 'end_offset' => 3]);
...
test('the ownership check lists a witness holding one work in two transcripts, and passes when clean', function () {
    ...
    foreach (['First', 'Second'] as $name) {
        ...
        Assignment::factory()->for($layer)->for($segment, 'segment')->create(['start_offset' => 0, 'end_offset' => 3]);
    }
...
test('the assignment check reports spans that drifted off their words, and passes when clean', function () {
    $scheme = ReferenceScheme::factory()->create();
    $work = Work::factory()->for($scheme, 'referenceScheme')->create();
    $one = Segment::factory()->for($work)->create(['address' => ['book' => 1, 'line' => 1], 'sort_key' => '00000001.00000001', 'label' => '1.1']);
    $two = Segment::factory()->for($work)->create(['address' => ['book' => 1, 'line' => 2], 'sort_key' => '00000001.00000002', 'label' => '1.2']);
    $layer = TranscriptionLayer::factory()->create(['text' => "the quick fox\nline two"]);
    Assignment::factory()->for($layer)->for($one, 'segment')->create(['start_offset' => 0, 'end_offset' => 13]);
    $second = Assignment::factory()->for($layer)->for($two, 'segment')->create(['start_offset' => 14, 'end_offset' => 22]);
...
test('a span that slips off its words is flagged at the save that did it, and unflagged when it is back', function () {
    ...
    $first = Assignment::factory()->for($layer)->for($one, 'segment')->create(['start_offset' => 0, 'end_offset' => 7]);
    $second = Assignment::factory()->for($layer)->for($two, 'segment')->create(['start_offset' => 8, 'end_offset' => 16]);
...
test('the assignment check can flag drifted spans in place', function () {
    ...
    $drifted = Assignment::factory()->for($layer)->for($two, 'segment')->create(['start_offset' => 13, 'end_offset' => 21]);
