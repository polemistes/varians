...
reads which side of a citation's marker the caret stands on and puts it on
...
So: the transcription editor's caret and citation-claim behaviour, and
`lib/transcriptionEdit.ts` — the client mirror of `SpanTransformer`, which
must agree with it case for case or a citation jumps the moment a save
