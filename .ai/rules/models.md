...
`EditionTransposition` (`{edition_id, conjecture_id}`) is the thin per-edition *adoption* of a transposition proposal — mirrors `EditionLemma`'s "a row's existence is the decision" pattern, except boolean rather than a selection among competing readings, since there's nothing to compete with. `EditionController::applyTranspositions()` walks an edition's adopted transpositions and reorders its passage list accordingly *before* windowing/pagination — an edition's own passage order can float free of citation `sort_key` order, the same way a single transcription's physical order already can (see the "Citation identity vs. reading order" note above); each passage keeps its own citation label wherever it lands, exactly like a transposed manuscript passage does.
