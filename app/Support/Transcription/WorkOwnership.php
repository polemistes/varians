...
class WorkOwnership
{
    ...
    public static function holder(Transcription $transcription, Work $work): ?Transcription
    {
        return Transcription::query()
            ...
            ->whereHas('layers.assignments.segment', fn ($query) => $query->where('work_id', $work->id))
    ...
    public static function violations(): Collection
    {
        $rows = DB::table('assignments')
            ...
            ->join('segments', 'segments.id', '=', 'assignments.segment_id')
            ->join('works', 'works.id', '=', 'segments.work_id')
